...
class Tree:
    ...
    def overflow(self, node):
        ...
        if not node.children:
            ...
            node.parent.keys.sort(key=lambda x: x[0])
            ...
            newroot.keys.sort(key=lambda x: x[0])
    ...
    def finds(self,lo,hi):#对一个范围进行查找
        found,current=self.find(lo)
        '''current=node.root
...
if __name__ == "__main__":
    ...
    basicSet = [(1,35), (2,14), (3,65), (4,64), (5,65),(6,85)]
